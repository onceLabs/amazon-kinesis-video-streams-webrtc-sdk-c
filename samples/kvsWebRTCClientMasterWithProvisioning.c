...
extern STATUS createCredentialAndKey( PCHAR claimCertPath, PCHAR claimKeyPath, PCHAR pIotCoreCert,
    PCHAR pIotCorePrivateKey,  PCHAR serialNum, PCHAR templete, PCHAR pEndpoint, PCHAR pRootCaCert );
...
INT32 main(INT32 argc, CHAR* argv[])
{
    ...
    printf("[KVS Master] Using provision template to get the thing credentials and private key\n");
#ifdef IOT_CORE_ENABLE_CREDENTIALS
    PCHAR pClaimeCert, pClaimPrivateKey, pIotCoreRoleAlias, pIotCoreThingName, pIotCoreCert, pIotCorePrivateKey, pSerialNum, pTemplate, pEndpoint, pRootCaCert;
    CHK_ERR((pClaimeCert = getenv(IOT_CORE_CLAIM_CERT)) != NULL, STATUS_INVALID_OPERATION, "AWS_IOT_CLAIM_CERT_PATH must be set");
    CHK_ERR((pClaimPrivateKey = getenv(IOT_CORE_CLAIM_PRIVATE_KEY)) != NULL, STATUS_INVALID_OPERATION, "AWS_IOT_CLAIM_PRIVATE_KEY_PATH must be set");
    ...
    CHK_ERR((pTemplate = getenv(IOT_CORE_PROVISIONING_TEMPLATE_NAME)) != NULL, STATUS_INVALID_OPERATION, "AWS_IOT_CORE_PROVISIONING_TEMPLATE_NAME must be set");
    CHK_ERR((pEndpoint = getenv(AWS_IOT_ENDPOINT)) != NULL, STATUS_INVALID_OPERATION, "AWS_IOT_ENDPOINT must be set");
    CHK_ERR((pRootCaCert = getenv(IOT_CORE_ROOT_CA_CERT)) != NULL, STATUS_INVALID_OPERATION, "AWS_IOT_CORE_ROOT_CA_CERT must be set");

    retStatus = createCredentialAndKey(pClaimeCert, pClaimPrivateKey, pIotCoreCert, pIotCorePrivateKey, pChannelName, pTemplate, pEndpoint, pRootCaCert);
    if (retStatus != STATUS_SUCCESS) {
        printf("[KVS Master] createCredentialAndKey(): operation returned status code: 0x%08x \n", retStatus);
        goto CleanUp;
    }
#else
