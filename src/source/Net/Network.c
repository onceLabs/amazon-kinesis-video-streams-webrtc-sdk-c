...
#ifdef _WIN32
    CHK_ERR(closesocket(sockfd) == 0, STATUS_NET_CLOSE_SOCKET_FAILED, "Failed to close the socket %s", getErrorString(getErrorCode()));
#else
...
STATUS socketConnect(PKvsIpAddress pPeerAddress, INT32 sockfd)
{
    ...
    CHK_ERR(retVal >= 0 || getErrorCode() == EINPROGRESS, STATUS_NET_SOCKET_CONNECT_FAILED, "connect() failed with errno %s",
...
STATUS getIpWithHostName(PCHAR hostname, PKvsIpAddress destIp)
{
    ...
    if (errCode != 0) {
        errStr = errCode == EAI_SYSTEM ? strerror(errno) : (PCHAR) getGaiStrRrror(errCode);
