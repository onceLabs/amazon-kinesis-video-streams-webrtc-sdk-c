...
static STATUS iceAgentInitSrflxCandidate(PIceAgent);
static STATUS iceAgentInitRelayCandidates(PIceAgent);
static STATUS iceAgentInitRelayCandidate(PIceAgent, UINT32, KVS_SOCKET_PROTOCOL);
static STATUS iceAgentCheckPeerReflexiveCandidate(PIceAgent pIceAgent, PKvsIpAddress pIpAddress, UINT32 priority, BOOL isRemote,
...
STATUS iceAgentValidateKvsRtcConfig(PKvsRtcConfiguration pKvsRtcConfiguration)
{
    ...
    DLOGD("\n\ticeLocalCandidateGatheringTimeout: %llu ms"
          "\n\ticeConnectionCheckTimeout: %llu ms"
          "\n\ticeCandidateNominationTimeout: %llu ms"
          "\n\ticeConnectionCheckPollingInterval: %llu ms",
...
STATUS insertIceCandidatePair(PDoubleList pIceCandidatePairs, PIceCandidatePair pNewPair)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    PDoubleListNode pCurNode = NULL;
 ...
 * @return STATUS status of execution
 */
static STATUS iceAgentInitSrflxCandidate(PIceAgent pIceAgent)
{
    STATUS retStatus = STATUS_SUCCESS;
    PDoubleListNode pCurNode = NULL;
    UINT64 data;
 ...
 * @return STATUS status of execution.
 */
static STATUS iceAgentInitRelayCandidates(PIceAgent pIceAgent)
{
    STATUS retStatus = STATUS_SUCCESS;
    UINT32 j;
 ...
 */
static STATUS iceAgentInitRelayCandidate(PIceAgent pIceAgent, UINT32 iceServerIndex, KVS_SOCKET_PROTOCOL protocol)
{
    STATUS retStatus = STATUS_SUCCESS;
    PDoubleListNode pCurNode = NULL;
    UINT64 data;
...
STATUS iceAgentConnectedStateSetup(PIceAgent pIceAgent)
{
    ...
    if (pIceAgent->pDataSendingIceCandidatePair != NULL) {
        ...
        if (IS_CANN_PAIR_SENDING_FROM_RELAYED(pLastDataSendingIceCandidatePair)) {
            ...
            CHK_STATUS(connectionListenerRemoveConnection(pIceAgent->pConnectionListener, pLastDataSendingIceCandidatePair->local->pSocketConnection));
                        ...
                        PKvsIpAddress pDestAddr)
{
    ...
    switch (stunPacketType) {
        ...
        case STUN_PACKET_TYPE_BINDING_RESPONSE_SUCCESS:
            ...
            CHK_WARN(transactionIdStoreHasId(pIceCandidatePair->pTransactionIdStore, pBuffer + STUN_PACKET_TRANSACTION_ID_OFFSET),
